...
public class PriceEstimationServiceTest {
    ...
    PriceEstimationService svc = new PriceEstimationService(routeClient, timeClient, surgeClient, taxClient, discountsClient, calculator);
    ...
    void setup() {
        ...
        r.detourFactor = 1.1;
        when(routeClient.estimate(any(Double.class), any(Double.class), any(Double.class), any(Double.class))).thenReturn(r);
        ...
        when(taxClient.estimateToll(any(Map.class))).thenReturn(Map.of("amount", 2.5));
    ...
    void formula_calculation_matches_expected() {
        ...
        req.pickup.lat = 37.0; req.pickup.lon = -122.0;
        req.dropoff = new PriceEstimateRequest.LocationDto();
        req.dropoff.lat = 37.5; req.dropoff.lon = -122.5;

        var quote = svc.estimatePrice(req);
        assertThat(quote).isNotNull();
        assertThat(quote.breakdown).containsKeys("distanceKm","estimatedMinutes","surgeMultiplier","tollAmount","discountPct","taxRate");
        // With cd=1.0, ct=0.5: distance = 11.0 km -> cd*d=11.0; ct*t=10.0 => base=21.0
        // after surge (1.5) => 31.5; + toll 2.5 => 34.0; after discount 20% => 27.2; after tax 8% => 29.376 -> rounded 29.38
